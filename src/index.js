const url = "https://fullstackfinalproject-b2gn.onrender.com/"; //"http://localhost:3005/";
...
$(document).ready(function () {
  ...
  $("#getactivity").click(function () {
    $.getJSON(url + "activity?type=" + type.value, (thing) => {
      ...
      $("#activity").append(
        '<p style="display:inline;">You can ' +
          thing.activity.toLowerCase() +
          "! </p>"
      );
      $("#activity").append(
  ...
  $.getJSON(url + "favorites/", (faves) => {
    ...
    faves.forEach((fave) => {
      ...
      $("body").on("click", "button#" + fave.key + ".edit", (t) => {
        console.log(
          "calling PUT on " + t.currentTarget.id + ": " + fave.activity
        );
